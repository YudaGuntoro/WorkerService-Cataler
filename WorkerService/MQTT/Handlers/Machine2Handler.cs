...
{
    public class Machine2Handler : BackgroundService
    {
        private readonly ILogger<Machine2Handler> _logger;
        ...
        public Machine2Handler(
            ILogger<Machine2Handler> logger,
            IMqttPublisher mqtt,
            IMapper mapper,
            MachineQuery Machine2Query,
        ...
        public async Task HandleMessageAsync(string topic, string actualMachinePayload, CancellationToken ct = default)
        {
            try
            {
                var machineData = JsonConvert.DeserializeObject<CZEC2_Machine_Data>(actualMachinePayload);
                if (machineData?.CZEC2_Data == null)
                {
                    _logger.LogWarning("[Machine2] czec2_Data tidak ditemukan dalam payload.");
                    return;
                }

                var now = DateTime.Now;
                var czec2 = machineData.CZEC2_Data;
                var cardNo = czec2.CardNo;
                var lineNo = czec2.LineNo;
                ...
                if (ShiftChecker is not null)
                {
                    ...
                    var before = czec2.MCStatus;
                    if (Mode == "GAP" && czec2.MCStatus == 4) // Jika Status CO saat Gap Inject ke SCH.DT
                    {
                        czec2.MCStatus = 2;
                    }
                ...
                try
                {
                    ...
                    int currentActual = Convert.ToInt32(czec2.Actual);
                    ...
                    _logger.LogInformation("czec2.Actual = {0} (decimal), currentActual = {1} (int),  currentActual1 = {2} (int)", czec2.Actual, currentActual);
                    ...
                    machineStatusDto.OKProduct = czec2.OKCount;
                    machineStatusDto.NGProduct = czec2.NGCount;
                    machineStatusDto.COCount = await _machineQuery.GetCOCountAsync(Convert.ToInt16(lineNo));

                    // g) StatusCode dari MCStatus (tetap isi)
                    var status = await _machineQuery.GetMachineStatusCodeAsync(czec2.MCStatus);
                    ...
                    if (cardNoParam is null)
                    {
                        ...
                        productionDetailsDto.ActualProduction = czec2.Actual;
                        productionDetailsDto.MachinePlan = czec2.Plan;
                        productionDetailsDto.SystemPlan = 0; // kalau cardNo gak ada, plan otomatis 0

                        _logger.LogWarning(
                            "[Machine2] CardNoParameters NULL untuk CardNo={CardNo}, LineNo={LineNo}. Payload full dipublish dengan nilai default 0.",
                            cardNo, lineNo);
                    }
                    else
                    {
                        ...
                        _mapper.Map(czec2, productionDetailsDto);
                        productionDetailsDto.ActualProduction = czec2.Actual;
                    }

                    _logger.LogWarning("[PCSD] Actual Prod {1}", czec2.Actual);
                    ...
                    await MarkStartRunTimeOnModelChangeAsync(
                        ...
                        Convert.ToInt32(czec2.MCStatus),       // ← angka dari mesin (RUN = 1)
                        currentActual,
                        ct);
                        ...
                        else
                        {
                            ...
                            if (planId is not null)
                            {
                                ...
                                if (createdPlan is not null)
                                {
                                    ...
                                    productionDetailsDto.MachinePlan = czec2.Plan;         // dari mesin
                    ...
                    {
                        _logger.LogWarning(
                            "[Machine2] CardNo {CardNo} <= 100, plan tidak diregister. Payload tetap dipublish.",
                            cardNo);
                    }
                    ...
                    cycleTimeDto.Result = (decimal?)((float)(czec2.COATResult / 10m));
                    cycleTimeDto.Target = (decimal?)((float)(czec2.COATTarget / 10m));
                    cycleTimeDto.A_COAT_1 = (float)czec2.ACOAT1st / 10f;
                    cycleTimeDto.A_COAT_2 = (float)czec2.ACOAT2nd / 10f;
                    cycleTimeDto.A_COAT_3 = (float)czec2.ACOAT3rd / 10f;
                    cycleTimeDto.B_COAT_1 = (float)czec2.BCOAT1st / 10f;
                    cycleTimeDto.B_COAT_2 = (float)czec2.BCOAT2nd / 10f;
                    cycleTimeDto.B_COAT_3 = (float)czec2.BCOAT3rd / 10f;

                    // Insert Ke mesin
                    await _machineQuery.StatusChangeInsertAndUpdateAsync(lineNo, czec2.MCStatus);

                    if (czec2.COATResult == 0)
                    {
                        cycleTimeDto.Judgement = "-";
                    }
                    else
                    {
                        cycleTimeDto.Judgement = czec2.COATResult <= czec2.COATTarget ? "OK" : "NG";
                    }
                    ...
                    try { await _mqtt.PublishAsync("machine2/ack", jsonFull); }
                    catch (Exception ex) { _logger.LogError(ex, "[Machine2] Gagal publish ACK(full)"); }
                }
            ...
            {
                _logger.LogError(ex, "[Machine2] Gagal parsing payload JSON");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "[Machine2] Terjadi error saat memproses pesan");
            }
        }
        ...
        {
            _logger.LogInformation("Machine2Handler background service started.");
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    var payload = Machine_State.CZEC2Json;
                    if (!string.IsNullOrWhiteSpace(payload))
                    {
                        await HandleMessageAsync("machine2/ack", payload, stoppingToken);
                    }
                    else
                    {
                        _logger.LogDebug("[Machine2Handler] Payload kosong, skip tick {Time}", DateTime.Now);
                    }
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "[Machine2Handler] Error saat loop publish");
                }

                await Task.Delay(1000, stoppingToken); // publish setiap 1 detik
            }

            _logger.LogInformation("Machine2Handler background service stopped.");
        }
    }
}